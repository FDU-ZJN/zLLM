...
def run_benchmark(tokenizer, model, prompts: list, max_new_tokens) -> dict:
    print(f"\n[Benchmark] 共 {len(prompts)} 条 prompt，开始吞吐量测试...")
    print("-" * 68)
    ...
    for i, item in enumerate(prompts):
        ...
        res    = infer_single(tokenizer, model, prompt, max_new_tokens=max_new_tokens)
...
def print_benchmark_results(benchmark_stats: dict):
    """打印吞吐量和延迟测试结果"""
    print("\n" + "=" * 80)
    print(" 吞吐量 & 延迟基准测试结果")
    print("=" * 80)
    ...
    metrics = [
        ...
        ("max_new_tokens 配置", "max_new_tokens_cfg"),
        ("整体吞吐率 (tokens/sec)", "overall_throughput_tps"),
    ...
    for label, key in metrics:
        val = benchmark_stats.get(key, "N/A")
...
if __name__ == "__main__":
    ...
    if wikitext_file.exists():
        ...
        print(f"[ERROR] WikiText 文件不存在: {args.wikitext_file}")
        perplexity_stats = None
    
    # 2. 加载 prompts 并运行吞吐量和延迟测试
    prompts = load_prompts(args.prompt_file)
    if not prompts:
        print("[ERROR] 没有加载到任何 prompt，无法进行吞吐量测试")
        exit(1)
    ...
    # 3. 打印吞吐量测试结果
    print_benchmark_results(benchmark_stats)
